...
//!
//! Based on the Athena (Cambridge version) and Athena++ MHD codes. Athena
//! originally was written in 2002-2005 by Jim Stone, Tom Gardiner, and Peter
//! Teuben, with many important contributions by many other developers after
//! that, i.e. 2005-2014.
//!
//! Athena++ was started in Jan 2014, with the core design developed 4-7/2014
//! during an extended visit to the KITP at UCSB by J. Stone. GR was implemented
//! by Chris White and AMR by Kengo Tomida 2014-2016, with contributions from
//! many others (esp. K. Felker) continuing after that.
//!
//! Athena (Kokkos version) is an outgrowth of the Athena-Parthenon
//! collaboration, and is a completely new implementation based on the Kokkos
//! performance-portability library (which is an external dependency required
//! for this version). It was started 6/2020 during the pandemic. As part of the
//! keep-it-simple design, only a fraction of the features of the C++ version
//! are implemented.
//========================================================================================
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 1; i < argc; i++) {
    ...
    if (*argv[i] == '-' && *(argv[i] + 1) != '\0' && *(argv[i] + 2) == '\0') {
      // check that command line options that require arguments actually have
      // them:
  ...
  if (restart_file.empty() && input_file.empty()) {
    ...
    std::cout << "### FATAL ERROR in " << __FILE__ << " at line " << __LINE__
              << std::endl
              << "Either an input or restart file must be specified."
              << std::endl
  ...
  // Start the wall clock timer. This is done here rather than in the Driver to
  // ensure that the time taken in ProblemGenerator is also captured.
  Kokkos::Timer timer;

  //--- Step 3.
  //--------------------------------------------------------------------------
  // Construct ParameterInput object and load data either from restart or input
  // file. With MPI, the input is read by every rank in parallel using MPI-IO.

  ParameterInput *pinput = new ParameterInput;
  IOWrapper infile, restartfile;
  // read parameters from restart file
  bool single_file_per_rank =
      false; // DBF: flag for single_file_per_rank for rst files
  if (res_flag) {
    ...
    if (!file_check.good()) {
      std::cerr << "Error: Unable to open restart file: " << restart_file
                << std::endl;
      // Handle the error (e.g., exit the program or use a default
      // configuration)
    }
  ...
  // read parameters from input file.  If both -r and -i are specified, this
  // will override parameters from the restart file
  if (iarg_flag) {
  ...
  //--- Step 4.
  //--------------------------------------------------------------------------
  // Construct Mesh.  Then build MeshBlockTree and add MeshBlockPack containing
  // MeshBlocks on this rank.  Latter cannot be performed in Mesh constructor
  // since it requires pointer to Mesh.
  ...
  //--- Step 5.
  //--------------------------------------------------------------------------
  // Add coordinates and physics modules to MeshBlockPack, and set initial
  // conditions. Note these steps must occur after Mesh (including MeshBlocks
  // and MeshBlockPack) is fully constructed.
  ...
  if (!res_flag) {
    ...
    // read ICs from restart file using ProblemGenerator constructor for
    // restarts
  ...
  //--- Step 6.
  //--------------------------------------------------------------------------
  // Construct Driver and Outputs. Actual outputs (including initial conditions)
  // are made in Driver.Initialize(). Add wall clock timer to Driver if
  // necessary.
  ...
  // Note anything containing a Kokkos::view must be deleted before
  // Kokkos::finalize()
