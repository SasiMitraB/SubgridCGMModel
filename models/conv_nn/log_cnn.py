...
def snapshot_pred(
    ...
    input_mean = np.load(
        os.path.join(MODEL_SAVE_DIR, "log_model_saves", f"cnn_{resolution}_{downsample}_input_mean.npy")
    )

    input_std = np.load(
        os.path.join(MODEL_SAVE_DIR, "log_model_saves", f"cnn_{resolution}_{downsample}_input_std.npy")
    )
    ...
    model_path = os.path.join(MODEL_SAVE_DIR, "log_model_saves", f"cnn_{resolution}_{downsample}.pth")
...
if __name__ == "__main__":
    ...
    np.save(os.path.join(MODEL_SAVE_DIR, "log_model_saves", f"cnn_{resolution}_{downsample}_input_mean.npy"),
            input_mean.cpu().numpy())
    np.save(os.path.join(MODEL_SAVE_DIR, "log_model_saves", f"cnn_{resolution}_{downsample}_input_std.npy"),
            input_std.cpu().numpy())
    ...
    torch.save(
        ...
        os.path.join(MODEL_SAVE_DIR, "log_model_saves", f"cnn_{resolution}_{downsample}.pth")
    )
    ...
    plt.savefig(
        os.path.join(LOSS_PLOT_DIR, "log_loss_plots", f"cnn_{resolution}_{downsample}_loss.jpg"),
        dpi=500
    )
